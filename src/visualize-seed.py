...
data = pd.read_csv('target/result_columns_seed.csv')

st.set_page_config(layout="wide")
model = st.sidebar.slider("Models: ", 0,  10, 0, 1)
samples = st.sidebar.slider("Samples: ", 1,  5, 3, 1)
top_results_to_compare = st.sidebar.slider("Top results to compare: ", 1,  10, 5, 1)
fairness_considuration_ratio = st.sidebar.slider("Fairness Consideration Ratio: ", 0.05,  0.25, 0.1, 0.05)
...
data_samples = data[(data['fairness_considuration_ratio'] == fairness_considuration_ratio) & (data['top_results_to_compare'] == top_results_to_compare) & (data['model'] == model)]
data_top_results_to_compare = data[(data['samples'] == samples) & (data['fairness_considuration_ratio'] == fairness_considuration_ratio) & (data['model'] == model)]
data_fairness_considuration_ratio = data[(data['samples'] == samples) & (data['top_results_to_compare'] == top_results_to_compare) & (data['model'] == model)]
